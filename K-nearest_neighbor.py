...
class KNearestNeighbor:
    ...
    def seeking_the_mini_coordinates(self):
        ...
        for i in range(0, 20):
            for j in range(0, 2):
                lis_mini.append(self.x[i][j])
        new_lis_mini_shape = []
        for i in range(0, 39, 2):
            new_lis_mini_shape.append([lis_mini[i], lis_mini[i+1]])
        distance = self.compute_Euclidean_distances()
        for i in range(0, 20-1):
            for j in range(0, 20-1-i):
                if(distance[j] > distance[j+1]):
                    distance[j], distance[j+1] = distance[j+1], distance[j]
                    new_lis_mini_shape[j], new_lis_mini_shape[j+1] = new_lis_mini_shape[j+1], new_lis_mini_shape[j]
        print(distance)
        print(new_lis_mini_shape)
        #Tìm cách hoán vị vị trí của mảng gần nhất theo khoảng cách nhỏ nhất
        ...
        for c in range(0, self.k):
            result.append(new_lis_mini_shape[c])
        return result
...
if __name__ == "__main__":
    ...
    for i in range(0, K):
        for j in range(0, 1):
            plt.plot(mini_coordinate[i][j], mini_coordinate[i][j+1], 'm*')
